...
def extract_data_from_images(images_base64):
    ...
    - **Actual CBM**: Actual CBM present on the invoice.
    - **PO No**: PO Number of the invoice.
...
def consolidate_results(batch_results):
    ...
    1. Invoice metadata (Inv No, Invoice Date, PO No, etc.)
...
#         save_dir = f"processed_images/{pdf_file.name.replace('.pdf', '')}"
        ...
        if "error" in result:
            ...
            for invoice_data in extracted_data:
                invoice_metadata = {
                    ...
                    "PO No": invoice_data.get("PO No", "None"),
                    ...
                    "Actual CBM": invoice_data.get("Actual CBM", "None"),
                }
